...
var card = (function () {
  ...
  function show(point) {
    _.forEach(point.cards, function (c) {
      ...
      // $(template.card(cardData)).appendTo($(elemCardContainer).empty()).perfectScrollbar();
      $(template.card(cardData)).prependTo($(elemCardContainer));
    });
